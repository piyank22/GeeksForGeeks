...
void euler(node *a,vector<int> &eulerPath, vector<int> &level, vector<int> &firstOcc, int l=0)
{
	if(a==NULL)
		return;
	else
	{
		if(firstOcc[ mapping[(a->d)]-1 ] == -1 )
			firstOcc[ mapping[(a->d)]-1 ] = eulerPath.size();
		eulerPath.push_back(a->d);
		level.push_back(l);
		euler(a->left,eulerPath,level,firstOcc,l+1);
		if(a->left != NULL)
		{
			eulerPath.push_back(a->d);
			level.push_back(l);
		}
		euler(a->right,eulerPath,level,firstOcc,l+1);
		if(a->right != NULL)
		{
			eulerPath.push_back(a->d);
			level.push_back(l);
...
void lca( vector<int> &eulerPath , vector<int> &level , vector<int> &firstOcc ,int n1, int n2)
{
	int i=firstOcc[mapping[n1]-1], j=firstOcc[mapping[n2]-1];

	// finding minimum level in level vectorB
	int min=level[i];
	int min_index=i;
	for(int k=i+1;k<=j;k++)
	{
	...
	cout<<"\nth lca of "<<n1<<" and "<<n2<<" is : "<<eulerPath[min_index];
}
...
int main()
{
    ...
    vector<int> level;
    vector<int> firstOcc(9,-1);

    // we made the mapping to give the nodes a sence of ordering // elements of the nodes must be distinct
    ...
    euler(root,eulerPath,level,firstOcc);

    cout<<"\nThe euler tour is:\n";
    for(int i=0;i<eulerPath.size();i++)
    	cout<<"\t"<<eulerPath[i];
    
    cout<<"\nThe level of euler tour is:\n";;
    for(int i=0;i<level.size();i++)
    	cout<<"\t"<<level[i];

    cout<<"\nThe first occuring position is:\n";
    for(int i=0;i<firstOcc.size();i++)
    	cout<<"\t"<<firstOcc[i];
	cout<<endl;

	lca(eulerPath,level,firstOcc,40,80);
